	...
	public unsafe override void CodeImage(IntPtr Scan0, Rectangle ScanArea, Size ImageSize, PixelFormat Format, Stream outStream)
	{
		lock (base.ImageProcessLock)
		{
			...
			if (EncodeBuffer == null)
			{
				...
				fixed (byte* encodeBuffer = EncodeBuffer)
				{
					byte[] array = null;
					...
					NativeMethods.memcpy(new IntPtr(encodeBuffer), Scan0, (uint)num2);
				}
				return;
			}
			...
			fixed (byte* encodeBuffer2 = EncodeBuffer)
			{
				for (int i = ScanArea.Y; i != ScanArea.Height; i += size.Height)
				{
					...
					if (NativeMethods.memcmp(encodeBuffer2 + num8, ptr + num8, (uint)num) != 0)
					{
						num5 = list.Count - 1;
				...
				for (; j < list.Count; j++)
				{
					...
					for (x = ScanArea.X; x != ScanArea.Width; x += size.Width)
					{
						...
						for (int k = 0; k < rectangle.Height; k++)
						{
							...
							if (NativeMethods.memcmp(encodeBuffer2 + num9, ptr + num9, (uint)count) != 0)
							{
								flag = true;
							}
							NativeMethods.memcpy(encodeBuffer2 + num9, ptr + num9, (uint)count);
						}
						if (onCodeDebugScan != null)
						{
							onCodeDebugScan(rectangle);
